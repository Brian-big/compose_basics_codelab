...
private fun Greetings(names: List<String> = List(1000) { "$it" }) {
    LazyColumn(modifier = Modifier.padding(vertical = 4.dp)) {// equivalent to recyclerViews
...
private fun Greeting(name: String) {
    ...
    val extraPadding by animateDpAsState( if (expanded.value) 48.dp else 0.dp,
        animationSpec = spring(Spring.DampingRatioMediumBouncy, stiffness = Spring.StiffnessVeryLow))
